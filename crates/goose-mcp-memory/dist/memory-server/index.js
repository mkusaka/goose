...
server.tool("remember_memory", "Stores a memory with optional tags in a specified category", {
...
server.tool("retrieve_memories", "Retrieves all memories from a specified category", {
    category: z.string().min(1, { message: 'Category must not be empty' }),
    is_global: z.boolean().default(false)
...
server.tool("remove_memory_category", "Removes all memories within a specified category", {
    category: z.string().min(1, { message: 'Category must not be empty' }),
    is_global: z.boolean().default(false)
}, async ({ category, is_global }) => {
    try {
...
server.tool("remove_specific_memory", "Removes a specific memory within a specified category", {
    category: z.string().min(1, { message: 'Category must not be empty' }),
...
async function retrieveCategory(category, isGlobal) {
